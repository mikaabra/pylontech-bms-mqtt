...
namespace esphome {
namespace esp32_can_listen {

static const char *const TAG = "esp32_can_listen";
...
bool ESP32CanListen::setup_internal() {
...
canbus::Error ESP32CanListen::send_message(struct canbus::CanFrame *frame) {
...
canbus::Error ESP32CanListen::read_message(struct canbus::CanFrame *frame) {
  if (this->twai_handle_ == nullptr) {
    // not setup yet or setup failed
    return canbus::ERROR_FAIL;
  }
...
bool ESP32CanListen::switch_to_normal_mode() {
  if (this->mode_ == CAN_MODE_NORMAL) {
    ESP_LOGI(TAG, "Already in NORMAL mode");
    return true;
  }

  if (this->twai_handle_ == nullptr) {
    ESP_LOGE(TAG, "Cannot switch mode: driver not initialized");
    return false;
  }

  ESP_LOGI(TAG, "Switching CAN bus from LISTEN_ONLY to NORMAL mode...");

  // Stop the driver
  if (twai_stop_v2(this->twai_handle_) != ESP_OK) {
    ESP_LOGE(TAG, "Failed to stop TWAI driver");
    return false;
  }

  // Uninstall the driver
  if (twai_driver_uninstall_v2(this->twai_handle_) != ESP_OK) {
    ESP_LOGE(TAG, "Failed to uninstall TWAI driver");
    return false;
  }

  this->twai_handle_ = nullptr;

  // Reinstall with NORMAL mode
  twai_general_config_t g_config =
      TWAI_GENERAL_CONFIG_DEFAULT((gpio_num_t) this->tx_, (gpio_num_t) this->rx_, TWAI_MODE_NORMAL);
  g_config.controller_id = 0;  // Reuse controller 0
  if (this->tx_queue_len_.has_value()) {
    g_config.tx_queue_len = this->tx_queue_len_.value();
  }
  if (this->rx_queue_len_.has_value()) {
  ...
  if (!get_bitrate(this->bit_rate_, &t_config)) {
    ESP_LOGE(TAG, "Invalid bit rate during mode switch");
    return false;
  }
  ...
  this->mode_ = CAN_MODE_NORMAL;
  ESP_LOGI(TAG, "CAN bus now in NORMAL mode (active, sending ACKs)");
  return true;
}
