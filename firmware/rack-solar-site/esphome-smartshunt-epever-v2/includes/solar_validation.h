...
// NOTE: safe_elapsed is defined in shared/hysteresis.h — must be included before this header.
// NOTE: safe_snprintf and PublishPacer are in shared/mqtt_helpers.h — must be included before this header.

static const uint8_t STABILITY_WINDOW_SIZE = 5;

// ============================================================================
// SLIDING WINDOW VALIDATOR (float)
...
struct StabilityWindow {
    ...
    uint8_t count;
    uint8_t index;
    float last_published;

    StabilityWindow() : count(0), index(0), last_published(NAN) {
        for (int i = 0; i < STABILITY_WINDOW_SIZE; i++) values[i] = NAN;
    }
};
...
inline void stability_window_add(StabilityWindow& window, float value) {
    ...
    if (window.count < STABILITY_WINDOW_SIZE) window.count++;
}
...
inline bool stability_window_is_stable(const StabilityWindow& window, float threshold) {
    if (window.count < STABILITY_WINDOW_SIZE) return window.count >= 3;
...
inline float stability_window_average(const StabilityWindow& window) {
    ...
    for (uint8_t i = 0; i < window.count; i++) {
        if (!std::isnan(window.values[i])) { sum += window.values[i]; valid_count++; }
    }
...
inline bool check_threshold_float_stable(float new_val, float& last_val, uint32_t& last_publish,
    ...
    stability_window_add(window, new_val);
    if (!stability_window_is_stable(window, stability_threshold)) return false;
...
struct IntStabilityWindow {
    ...
    IntStabilityWindow() : count(0), index(0), last_published(-1) {
        for (int i = 0; i < STABILITY_WINDOW_SIZE; i++) values[i] = -1;
    }
};
...
inline int int_stability_window_average(const IntStabilityWindow& window) {
    ...
    for (uint8_t i = 0; i < window.count; i++) {
        if (window.values[i] >= 0) { sum += window.values[i]; valid_count++; }
    }
    return valid_count > 0 ? (sum + valid_count / 2) / valid_count : -1;
}

inline bool int_stability_window_is_stable(const IntStabilityWindow& window, int max_range) {
    if (window.count < STABILITY_WINDOW_SIZE) return window.count >= 3;
    int min_val = window.values[0], max_val = window.values[0];
...
inline bool check_threshold_int_stable(int new_val, int& last_val, uint32_t& last_publish,
    ...
    int_stability_window_add(window, new_val);
    if (!int_stability_window_is_stable(window, max_stability_range)) return false;
...
// ============================================================================
// DEBOUNCE THRESHOLD (bool)
// ============================================================================

inline bool check_threshold_bool(bool new_val, bool &last_val, uint32_t &last_change_time,
                                  bool &pending_val, bool &has_pending) {
    uint32_t now = millis();
    ...
    if (has_pending && new_val != pending_val) has_pending = false;
    return false;
}
...
inline bool is_valid_printable(const std::string& s) {
    for (char c : s) { if (c < 32 || c > 126) return false; }
    return true;
}
...
inline bool validate_model_description(const std::string& s) {
    ...
    return (s.find("SmartShunt") != std::string::npos || s.find("BMV") != std::string::npos);
}
...
inline bool validate_serial_number(const std::string& s) {
    ...
    for (char c : s) { if (!std::isalnum(c) && c != '-') return false; }
    return true;
}
...
inline bool validate_alarm_condition(const std::string& s) {
    ...
    std::string lower;
    for (char c : s) lower += std::tolower(c);
    if (lower.length() <= 3) return (lower[0] == 'o' || lower.find("alarm") != std::string::npos);
    return (lower == "on" || lower == "off" ||
            lower.find("alarm") != std::string::npos || lower.find("ok") != std::string::npos);
}
...
inline void record_bitflip_event(uint32_t& count, uint32_t& window_start, uint32_t now) {
    const uint32_t WINDOW_MS = 600000;
    if (window_start == 0) { window_start = now; count = 1; return; }
    uint32_t elapsed = safe_elapsed(now, window_start);
    if (elapsed >= WINDOW_MS) { count = 1; window_start = now; }
    else count++;
}

inline float get_bitflip_rate_per_minute(uint32_t count, uint32_t window_start, uint32_t now) {
    if (count == 0 || window_start == 0) return 0.0f;
    const uint32_t WINDOW_MS = 600000;
    uint32_t elapsed = safe_elapsed(now, window_start);
    if (elapsed >= WINDOW_MS) return 0.0f;
    float elapsed_minutes = elapsed / 60000.0f;
    if (elapsed_minutes < 0.1f) elapsed_minutes = 0.1f;
